...
from rasim_next.pipeline._continuous_detector_kernel import compiled_cif_factor_keys
...
def build_source_averaged_detector(
    ...
    """Compile the optimized finite-stack renderer for supported structure models.

    Generic CIF compilation is exact for one or two distinct element/charge
    scattering-factor groups. Larger chemistries fail before detector state is
    allocated and must use the sparse structure detector.
    """

    if not isinstance(inputs.strength, (Bi2X3FiniteStackStrength, CifFiniteStackStrength)):
        raise TypeError(
            "the optimized renderer requires a supported finite-stack strength model; use "
            "build_source_averaged_structure_detector for other structures"
        )
    if isinstance(inputs.strength, CifFiniteStackStrength):
        compiled_cif_factor_keys(inputs.strength)
    reachable_rods = _source_reachable_rods(inputs)
    incidence_axis_angle_rad, scan_binding_revision = _configured_incidence_scan_metadata(inputs)
...
def sample_reciprocal_space(inputs: ConfiguredSimulationInputs) -> ReciprocalSpaceDisplay:
    ...
    for rod in inputs.bragg_space.config.rods:
        ...
        rod_alpha = alpha if rod.family_m == 0 else np.concatenate((-alpha[::-1], alpha))
        alpha_grid, beta_grid, axial_grid = np.meshgrid(rod_alpha, beta, axial, indexing="ij")
...
def evaluate_nominal_ewald_surface(
    ...
    *,
    ...
    for rod in context.geometry.coating.bragg_space.config.rods:
        rod_alpha = alpha if rod.family_m == 0 else np.concatenate((-alpha[::-1], alpha))
        alpha_grid, beta_grid = np.meshgrid(rod_alpha, beta, indexing="ij")
