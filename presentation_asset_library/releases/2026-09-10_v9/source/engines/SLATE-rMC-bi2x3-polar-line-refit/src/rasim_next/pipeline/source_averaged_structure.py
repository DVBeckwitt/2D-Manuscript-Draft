...
class SourceAveragedDetectorStructureResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if np.any(term_is_m0 & (term_alpha < 0.0)):
            raise ValueError("m=0 sparse-response alpha values must be folded")
        ...
        if self.orientation_model_id != ROD_POLAR_LINE_MOSAIC_MODEL_ID:
            raise ValueError("unsupported sparse-response orientation model")
        if self.coordinate_semantics_id != ROD_POLAR_LINE_COORDINATE_SEMANTICS_ID:
            raise ValueError("unsupported sparse-response coordinate semantics")
        if self.probability_measure_id != ROD_POLAR_LINE_PROBABILITY_MEASURE_ID:
            raise ValueError("unsupported sparse-response probability measure")
        if self.measure_id != (
        ...
        response_revision = canonical_revision_sha256(
            ("definition_id", "source_averaged_sparse_structure_response.v5"),
    ...
    def apply_strength_for_mosaics(
        ...
        *,
        ...
        m0_term = np.asarray(
            [self.rods[index].family_m == 0 for index in self.term_rod_index],
            dtype=np.bool_,
        )
        angular_factor = np.where(m0_term, 1.0 / np.pi, 1.0 / (2.0 * np.pi))
        return tuple(
            self._coordinate_intensity(
                fixed
                * wrapped_mosaic_line_density_rad_inv(self.term_alpha_rad, mosaic)
                * angular_factor
            )
...
def _source_averaged_sparse_fixed_physics_revision(
    *,
    ...
    return canonical_revision_sha256(
        ("definition_id", "source_averaged_sparse_fixed_physics.v4"),
...
def compile_source_averaged_detector_structure_response(
    ...
    *,
    ...
    for supplied_state_index in valid_state_index:
        ...
        if (
            state_response.orientation_model_id != ROD_POLAR_LINE_MOSAIC_MODEL_ID
            or state_response.coordinate_semantics_id
            != ROD_POLAR_LINE_COORDINATE_SEMANTICS_ID
            or state_response.probability_measure_id
            != ROD_POLAR_LINE_PROBABILITY_MEASURE_ID
        ):
            raise ValueError("source-state response uses incompatible orientation coordinates")
