...
class DetectorStructureResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or np.any((term_alpha <= -np.pi) | (term_alpha > np.pi))
        ...
        term_is_m0 = np.asarray(
            [rods[index].family_m == 0 for index in term_rod],
            dtype=np.bool_,
        )
        if np.any(term_is_m0 & (term_alpha < 0.0)):
            raise ValueError("m=0 structure-response alpha values must be folded")
        ...
        if self.orientation_model_id != ROD_POLAR_LINE_MOSAIC_MODEL_ID:
            raise ValueError("unsupported structure-response orientation model")
        if self.coordinate_semantics_id != ROD_POLAR_LINE_COORDINATE_SEMANTICS_ID:
            raise ValueError("unsupported structure-response coordinate semantics")
        if self.probability_measure_id != ROD_POLAR_LINE_PROBABILITY_MEASURE_ID:
            raise ValueError("unsupported structure-response probability measure")
...
def _compile_detector_state(
    *,
    ...
    if not isinstance(strength_model, (Bi2X3FiniteStackStrength, CifFiniteStackStrength)):
        raise TypeError("compiled integration requires a supported finite-stack strength model")
    ...
    if isinstance(strength_model, Bi2X3FiniteStackStrength):
        ...
        try:
            ...
            raise ValueError(
                "compiled integration does not implement stacking parent "
                f"{strength_model.parent.value}"
            ) from error
        shared_disorder_epsilon = strength_model.shared_disorder_epsilon
        structure_l_scale = 1.0
    else:
        stacking_parent_code = 0
        shared_disorder_epsilon = 0.0
        structure_l_scale = 3.0
    ...
    if packed_structure is None:
        if isinstance(strength_model, Bi2X3FiniteStackStrength):
            ...
            packed = pack_cif_finite_stack_structures(
                strength_model,
                wavelength_A=np.asarray([wavelength_A], dtype=np.float64),
            )
            packed_structure = (*packed[:3], packed[3][0], *packed[4:])
    (
        atom_offsets,
        atom_properties,
        f0_parameters,
    ...
    rod_inverse_constants = np.column_stack(
        (
            np.zeros(len(rods), dtype=np.float64),
    ...
    return CompiledDetectorState(
        ...
        **stitch_values,
        ...
        b3_norm_Ainv=float(np.linalg.norm(basis[:, 2]) / structure_l_scale),
...
class DetectorEwaldMeasure:
    ...
    def _inverse_rod_density(
        ...
        *,
        ...
        is_m0 = rod.family_m == 0
        x_squared = (
            (transverse_norm - abs(b)) * (transverse_norm + abs(b))
            if is_m0
        ...
        w_caustic = np.abs(q_norm - parallel_norm) <= reconstruction_tolerance
        x_squared = np.where(x_caustic, 0.0, x_squared)
        w_squared = np.where(w_caustic, 0.0, w_squared)
        ...
        rod_azimuth = float(np.arctan2(b, a))
        ...
        for x_sign in (-1.0, 1.0):
            ...
            if is_m0:
                ...
                radial_sign_phase = 0.0 if x_sign > 0.0 else np.pi
                beta = np.remainder(
                    azimuth_q - rod_azimuth - radial_sign_phase,
                    two_pi,
                )
            ...
            for w_sign in (-1.0, 1.0):
                ...
                if is_m0:
                    ...
                    alpha = (
                        np.remainder(
                            np.arctan2(w_value, parallel_norm)
                            - np.arctan2(q_local[:, 2], x_value)
                            + np.pi,
                            two_pi,
                        )
                        - np.pi
                    ...
                    alpha = np.where(
                        alpha <= -np.pi + angular_tolerance,
                        np.pi,
                        alpha,
                    )
                    folded = supported.copy()
                u_value = w_value - c0
                ...
                direction_local = np.column_stack(
                    (
                        np.sin(alpha) * np.cos(direction_azimuth),
                        np.sin(alpha) * np.sin(direction_azimuth),
                        np.cos(alpha),
                    )
                )
                ...
                singular_candidate = folded & (x_caustic | w_caustic)
                ...
                regular = folded & ~singular_candidate
    ...
    def _compiled_evaluator(self, rods: tuple[Rod, ...]) -> CompiledDetectorEvaluator:
        ...
        if not isinstance(strength, (Bi2X3FiniteStackStrength, CifFiniteStackStrength)):
            raise TypeError(
                "adaptive_compiled integration requires a supported finite-stack strength model"
            )
        ...
        *,
        ...
        for row_start in range(0, rows, row_chunk_size):
            ...
            local_candidate |= all_valid & (
                (np.min(q_local[..., 0], axis=-1) <= 0.0)
                & (np.max(q_local[..., 0], axis=-1) >= 0.0)
                & (np.min(q_local[..., 1], axis=-1) <= 0.0)
                & (np.max(q_local[..., 1], axis=-1) >= 0.0)
            )
            for rod in rods:
