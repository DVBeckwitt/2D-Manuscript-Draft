...
def pack_bi2se3_two_h_structures(
    ...
    *,
    ...
    factor_keys = tuple(
        sorted(
            {(atom.element, atom.charge) for atom in atoms},
            key=lambda value: (value[0] != "Bi", value[0], value[1]),
        )
    )
    if len(factor_keys) != 2 or factor_keys[0][0] != "Bi":
        raise ValueError("compiled quintuple-layer integration requires Bi and one chalcogen")
    factor_index = {key: position for position, key in enumerate(factor_keys)}
    ...
    properties = np.asarray(
        [
            (
                atom.occupancy,
                factor_index[(atom.element, atom.charge)],
    ...
    parameters, anomalous = _pack_scattering_factor_tables(factor_keys, wavelength)
    ...
    for value in (offsets, properties):
        value.setflags(write=False)
    return (
        offsets,
        properties,
...
def _pack_scattering_factor_tables(
    factor_keys: tuple[tuple[str, int], ...],
    wavelength_A: NDArray[np.float64],
) -> tuple[FloatArray, NDArray[np.complex128]]:
    """Pack at most two distinct charged-element scattering-factor rows."""

    if not 1 <= len(factor_keys) <= 2:
        raise ValueError("compiled finite-stack integration supports one or two factor groups")
    padded_keys = factor_keys if len(factor_keys) == 2 else (*factor_keys, factor_keys[0])
    waasmaier = xraydb.get_xraydb().get_cache("Waasmaier")
    parameters = np.empty((2, 11), dtype=np.float64)
    anomalous = np.empty((wavelength_A.size, 2), dtype=np.complex128)
    energy_eV = HC_EV_A / wavelength_A
    for position, (element, charge) in enumerate(padded_keys):
        species = _f0_species(element, charge)
    ...
    for value in (parameters, anomalous):
        value.setflags(write=False)
    return parameters, anomalous


def compiled_cif_factor_keys(
    strength: CifFiniteStackStrength,
) -> tuple[tuple[str, int], ...]:
    """Return the factor groups admitted by the two-row compiled kernel."""

    if not isinstance(strength, CifFiniteStackStrength):
        raise TypeError("compiled CIF integration requires CifFiniteStackStrength")
    factor_keys = tuple(
        sorted(
            {(site.element, site.charge) for site in strength.crystal.sites},
            key=lambda value: (value[0] != "Bi", value[0], value[1]),
        )
    )
    if not 1 <= len(factor_keys) <= 2:
        raise ValueError(
            "compiled CIF integration supports one or two distinct element/charge factor groups"
        )
    return factor_keys


def pack_cif_finite_stack_structures(
    strength: CifFiniteStackStrength,
    *,
    wavelength_A: NDArray[np.float64],
) -> tuple[
    ...
    """Pack a conventional-cell CIF repeat into the period-three compiled kernel.

    The kernel's layer phase is parameterized in thirds of its supplied rod
    coordinate.  Supplying ``L' = 3 L`` and ``z' = z / 3`` preserves every
    atomic phase while its AA repeat factor becomes the exact conventional-cell
    factor ``sum(exp(2 pi i n L), n=0..N-1)``.
    """
    ...
    sites = strength.crystal.sites
    factor_keys = compiled_cif_factor_keys(strength)
    factor_index = {key: position for position, key in enumerate(factor_keys)}
    parameters, anomalous = _pack_scattering_factor_tables(factor_keys, wavelength)
    offsets = np.asarray(
        [(site.fractional[0], site.fractional[1], site.fractional[2] / 3.0) for site in sites],
        dtype=np.float64,
    )
    properties = np.asarray(
        [
            (
                site.occupancy,
                factor_index[(site.element, site.charge)],
                strength.unknown_u_iso_A2 if site.u_iso_A2 is None else site.u_iso_A2,
                strength.unknown_u_iso_A2 if site.u_iso_A2 is None else site.u_iso_A2,
            )
            for site in sites
        ],
    ...
    if np.any(~np.isfinite(properties[:, 2:])):
        raise ValueError("compiled CIF integration requires resolved isotropic displacement")
    divisor = (
        float(strength.repeats)
        if strength.normalization is EventIntensityNormalization.FINITE_PER_LAYER
        else 1.0
    )
    for value in (offsets, properties):
        value.setflags(write=False)
    return offsets, properties, parameters, anomalous, strength.repeats, divisor, 0.0, 0.0
...
def _evaluate_point_into(
    ...
    for rod_index in range(rod_count):
        ...
        x_squared = (
            (transverse_norm - abs_b) * (transverse_norm + abs_b)
            if is_m0
            else transverse_norm * transverse_norm
        )
        ...
        if x_squared < -inverse_tolerance or w_squared < -inverse_tolerance:
            continue
        x_caustic = transverse_norm <= reconstruction_tolerance
        w_caustic = abs(q_norm - parallel_norm) <= reconstruction_tolerance
        if x_caustic:
            x_squared = 0.0
        if w_caustic:
            w_squared = 0.0
        ...
        rod_azimuth = math.atan2(b, a)
        for x_sign in (-1.0, 1.0):
            x_value = x_sign * x_magnitude
            if is_m0:
                beta = (azimuth_q - math.atan2(b, x_value)) % two_pi
            else:
                radial_sign_phase = 0.0 if x_sign > 0.0 else math.pi
                beta = (azimuth_q - rod_azimuth - radial_sign_phase) % two_pi
            if beta >= two_pi:
                beta = 0.0
            for w_sign in (-1.0, 1.0):
                ...
                if is_m0:
                    ...
                    alpha = (
                        math.atan2(w_value, parallel_norm)
                        - math.atan2(q_local_z, x_value)
                        + math.pi
                    ) % two_pi - math.pi
                    if alpha <= -math.pi + angular_tolerance:
                        alpha = math.pi
                ...
                if u_value < lower_u - u_tolerance or u_value > upper_u + u_tolerance:
                    continue
                sin_alpha = math.sin(alpha)
                direction_azimuth = beta if is_m0 else beta + rod_azimuth
                direction_local_x = sin_alpha * math.cos(direction_azimuth)
                direction_local_y = sin_alpha * math.sin(direction_azimuth)
                ...
                if x_caustic or w_caustic:
                    if is_m0:
                        ...
                        cos_direction_azimuth = math.cos(direction_azimuth)
                        sin_direction_azimuth = math.sin(direction_azimuth)
                        reconstructed_local_x = x_value * cos_direction_azimuth
                        reconstructed_local_y = x_value * sin_direction_azimuth
                        reconstructed_local_z = -parallel_norm * math.sin(
                            alpha
                        ) + w_value * math.cos(alpha)
...
def _accumulate_latent_pixel_mass_kernel(
    ...
    for draw in range(draw_count):
        ...
        for rod_index in range(rod_count):
            ...
            parallel_norm = state.rod_inverse_constants[rod_index, 1]
            rod_azimuth = math.atan2(b, a)
            direction_azimuth = beta + rod_azimuth
            sin_direction_azimuth = math.sin(direction_azimuth)
            cos_direction_azimuth = math.cos(direction_azimuth)
            direction_local_x = sin_alpha * cos_direction_azimuth
            direction_local_y = sin_alpha * sin_direction_azimuth
            ...
            x0 = parallel_norm * cos_alpha + c0 * sin_alpha
            q0_local_x = x0 * cos_direction_azimuth
            q0_local_y = x0 * sin_direction_azimuth
            q0_local_z = -parallel_norm * sin_alpha + c0 * cos_alpha
            q0_sample_x = (
    ...
    def accumulate_latent_pixel_mass(
        ...
        *,
        ...
        if (
            ...
            or np.any((alpha <= -np.pi) | (alpha > np.pi))
...
__all__ = [
    ...
    "pack_cif_finite_stack_structures",
]
