...
FIT_PARAMETER_SEED_SCHEMA = "rasim-layered-material-fit-seed-v3"
FIT_MOSAIC_INITIAL_GUESS_SCHEMA = "rasim-mosaic-initial-guess-v1"
...
@dataclass(frozen=True, slots=True)
class FitMosaicInitialGuess:
    """Numerical mosaic start that cannot cross a fixed-physics handoff."""

    gaussian_sigma_deg: float
    lorentzian_hwhm_deg: float
    lorentzian_probability: float
    provenance: str
    source_orientation_model_id: str
...
def _load_mosaic_initial_guess(record: object) -> FitMosaicInitialGuess:
    required = {
        "schema_version",
        "status",
        "gaussian_sigma_deg",
        ...
        "provenance",
        "source_orientation_model_id",
        "target_orientation_model_id",
        "target_coordinate_semantics_id",
        "target_probability_measure_id",
    }
    if not isinstance(record, dict) or set(record) != required:
        raise ValueError("mosaic initial guess has an invalid shape")
    if (
        record["schema_version"] != FIT_MOSAIC_INITIAL_GUESS_SCHEMA
        or record["status"] != "INITIAL_GUESS_ONLY"
        or record["source_orientation_model_id"] != "painted_ewald.folded_alpha_full_beta.v1"
        or record["target_orientation_model_id"] != ROD_POLAR_LINE_MOSAIC_MODEL_ID
        or record["target_coordinate_semantics_id"] != ROD_POLAR_LINE_COORDINATE_SEMANTICS_ID
        or record["target_probability_measure_id"] != ROD_POLAR_LINE_PROBABILITY_MEASURE_ID
    ):
        raise ValueError("mosaic initial guess has an unsupported scientific contract")
    gaussian = _finite_json_number(record["gaussian_sigma_deg"], "Gaussian mosaic start")
    lorentzian = _finite_json_number(record["lorentzian_hwhm_deg"], "Lorentzian mosaic start")
    probability = _finite_json_number(
        record["lorentzian_probability"], "Lorentzian mosaic probability start"
    )
    if gaussian <= 0.0 or lorentzian <= 0.0 or not 0.0 <= probability <= 1.0:
        raise ValueError("mosaic initial guess values are invalid")
    return FitMosaicInitialGuess(
        gaussian_sigma_deg=gaussian,
        lorentzian_hwhm_deg=lorentzian,
        lorentzian_probability=probability,
        provenance=_nonempty_string(record["provenance"], "mosaic initial guess provenance"),
        source_orientation_model_id=record["source_orientation_model_id"],
    )
...
def _load_fit_parameter_seed(
    ...
    *,
    ...
    return FitParameterSeed(
        ...
        mosaic=_load_mosaic_initial_guess(document.get("mosaic")),
