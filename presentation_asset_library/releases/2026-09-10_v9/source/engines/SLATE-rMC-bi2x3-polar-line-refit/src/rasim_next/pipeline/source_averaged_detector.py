...
def _compile_source_parratt_stitch(
    ...
    strength: Bi2X3FiniteStackStrength | CifFiniteStackStrength,
    rods: tuple[Rod, ...],
    *,
    wavelength_A: float,
...
def _incidence_angle_static_physics_revision(
    *,
    ...
    fields: list[tuple[str, object]] = [
        ("definition_id", "incidence_angle_static_detector_physics.v3"),
...
class MonteCarloDetectorPixelMass:
    ...
    proposal_id: str = field(init=False, default="signed_rod_polar_line_full_beta.v1")
    orientation_model_id: str = field(init=False, default=ROD_POLAR_LINE_MOSAIC_MODEL_ID)
    coordinate_semantics_id: str = field(
        init=False,
        default=ROD_POLAR_LINE_COORDINATE_SEMANTICS_ID,
    )
    probability_measure_id: str = field(
        init=False,
        default=ROD_POLAR_LINE_PROBABILITY_MEASURE_ID,
    )
...
def _sample_mosaic_orientation_matrix(
    ...
    *,
    ...
    wrapped = np.where(wrapped <= -pi, pi, wrapped)
    alpha = np.ascontiguousarray(wrapped.T)
...
class SourceAveragedDetectorEwaldMeasure:
    ...
    def __init__(
        ...
        *,
        ...
        rod_catalog_revision: str,
        mosaic: MosaicParameters,
        strength_model: Bi2X3FiniteStackStrength | CifFiniteStackStrength,
        ...
        if not isinstance(strength_model, (Bi2X3FiniteStackStrength, CifFiniteStackStrength)):
            raise TypeError("strength_model must be a supported finite-stack strength model")
        ...
        (
            ...
            if isinstance(strength_model, Bi2X3FiniteStackStrength)
            else pack_cif_finite_stack_structures(
                strength_model,
                wavelength_A=states.wavelength_A[valid_state_index],
            )
        )
    ...
    @property
    def strength_model(self) -> Bi2X3FiniteStackStrength | CifFiniteStackStrength:
    ...
    def rebind_physics(
        ...
        *,
        ...
        strength_model: Bi2X3FiniteStackStrength | CifFiniteStackStrength | None = None,
        intensity_envelope: SampleQIntensityEnvelope | None = None,
    ) -> SourceAveragedDetectorEwaldMeasure:
        """Replace admissible physics arrays while retaining combined source geometry.

        Generic CIF state may change mosaic or the external intensity envelope.
        A supplied CIF strength must retain the identical crystal object, repeat
        count, normalization, and unknown-displacement state because in-plane atom
        factors are geometry-bound. Candidate CIF structures use the sparse
        structure-response path instead.
        """
        ...
        if not isinstance(rebound_mosaic, MosaicParameters):
            raise TypeError("mosaic must be MosaicParameters")
        if not isinstance(rebound_strength, (Bi2X3FiniteStackStrength, CifFiniteStackStrength)):
            raise TypeError("strength_model must be a supported finite-stack strength model")
        if not isinstance(rebound_envelope, SampleQIntensityEnvelope):
            raise TypeError("intensity_envelope must be SampleQIntensityEnvelope")
        reference = self._strength_model
        if type(rebound_strength) is not type(reference):
            raise ValueError("physics rebinding cannot change the finite-stack model family")
        if isinstance(reference, Bi2X3FiniteStackStrength):
            invariant_structure = (
                isinstance(rebound_strength, Bi2X3FiniteStackStrength)
                and rebound_strength.crystal is reference.crystal
                and rebound_strength.layers == reference.layers
                and rebound_strength.parent is reference.parent
                and rebound_strength.normalization == reference.normalization
            )
            shared_disorder_epsilon = rebound_strength.shared_disorder_epsilon
        else:
            invariant_structure = (
                isinstance(rebound_strength, CifFiniteStackStrength)
                and rebound_strength.crystal is reference.crystal
                and rebound_strength.repeats == reference.repeats
                and rebound_strength.normalization == reference.normalization
                and rebound_strength.unknown_u_iso_A2 == reference.unknown_u_iso_A2
            )
            shared_disorder_epsilon = 0.0
        if not invariant_structure:
            qualifier = "generic CIF " if isinstance(reference, CifFiniteStackStrength) else ""
            raise ValueError(
                f"{qualifier}physics rebinding requires unchanged crystal, repeat topology, "
                "normalization, and displacement state"
            )
        ...
        (
            ...
            if isinstance(rebound_strength, Bi2X3FiniteStackStrength)
            else pack_cif_finite_stack_structures(
                rebound_strength,
                wavelength_A=self._incident.states.wavelength_A[valid_state_index],
            )
