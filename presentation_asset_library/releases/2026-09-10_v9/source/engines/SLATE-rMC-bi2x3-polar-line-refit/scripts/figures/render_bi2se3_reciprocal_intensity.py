...
pushes the accepted folded-m0/signed-nonzero rod-local measure and the same per-rod strength into
...
from painted_ewald import (  # noqa: E402
...
CACHE_SCHEMA = "rasim-bi2se3-axisymmetric-reciprocal-display-cache-v4"
VOLUME_ALGORITHM = "folded-m0-signed-nonzero-polar-line-finite-volume-display-v4"
...
class _RodHistogramTask:
    ...
    axial_Ainv: FloatArray
    signed_tilt_rad: FloatArray
    beta_rad: FloatArray
    orientation_probability_mass: FloatArray
...
def _deposit_rod_histogram(task: _RodHistogramTask) -> tuple[int, FloatArray]:
    rod = Rod(task.rod_h, task.rod_k)
    mean_axis_sample = task.crystal_to_sample @ task.reciprocal_basis_Ainv[:, 2]
    mean_axis_sample /= np.linalg.norm(mean_axis_sample)
    histogram = np.zeros(
    ...
    for start in range(0, task.axial_Ainv.size, AXIAL_TRANSFORM_CHUNK_SIZE):
        ...
        mapper_arguments = {
            "rod": rod,
            "reciprocal_basis_Ainv": task.reciprocal_basis_Ainv,
            "crystal_to_sample": task.crystal_to_sample,
            "beta_rad": task.beta_rad[:, None],
            "u_Ainv": task.axial_Ainv[None, start:stop],
        }
        if task.family_m == 0:
            q_sample = map_tied_rotation_latent(
                alpha_rad=task.signed_tilt_rad[:, None],
                **mapper_arguments,
            )
        else:
            q_sample = map_rod_polar_line_latent(
                signed_tilt_rad=task.signed_tilt_rad[:, None],
                **mapper_arguments,
            )
        qz = np.einsum("aui,i->au", q_sample, mean_axis_sample, optimize=True)
        radial_vector = q_sample - qz[:, :, None] * mean_axis_sample[None, None, :]
        qr = np.linalg.norm(radial_vector, axis=-1)
        weights = _mosaic_structure_mass_A(
            task.orientation_probability_mass,
...
def _axisymmetric_rod_mosaic_quadrature(
    *,
    rod: Rod,
    folded_alpha_rad: FloatArray,
    folded_probability_mass: FloatArray,
) -> tuple[FloatArray, FloatArray]:
    """Lift folded quadrature only where the nonzero-rod chart is signed."""

    alpha = np.asarray(folded_alpha_rad, dtype=np.float64)
    mass = np.asarray(folded_probability_mass, dtype=np.float64)
    if alpha.shape != mass.shape or alpha.ndim != 1:
        raise ValueError("folded mosaic coordinates and masses must be aligned vectors")
    if rod.family_m == 0:
        return alpha, mass
    positive = alpha > 0.0
    signed_tilt = np.concatenate((-alpha[positive][::-1], alpha))
    signed_mass = np.concatenate((0.5 * mass[positive][::-1], mass.copy()))
    signed_mass[positive.sum() :] *= np.where(alpha > 0.0, 0.5, 1.0)
    return signed_tilt, signed_mass
...
def compute_axisymmetric_display_volume(
    ...
    for family_m in PHYSICAL_FAMILIES:
        for rod in rods_by_family[family_m]:
            ...
            signed_tilt, probability_mass = _axisymmetric_rod_mosaic_quadrature(
                rod=rod,
                folded_alpha_rad=alpha_rad,
                folded_probability_mass=alpha_mass,
            )
            tasks.append(
                _RodHistogramTask(
                    family_m=family_m,
                    rod_h=rod.h,
                    rod_k=rod.k,
                    reciprocal_basis_Ainv=np.asarray(inputs.reciprocal.basis_Ainv, dtype=np.float64),
                    crystal_to_sample=np.asarray(
                        inputs.bragg_space.config.crystal_to_sample,
                        dtype=np.float64,
                    ),
                    axial_Ainv=np.asarray(axial, dtype=np.float64),
                    signed_tilt_rad=signed_tilt,
                    beta_rad=np.zeros_like(signed_tilt),
                    orientation_probability_mass=probability_mass,
