...
def _rod_display_angle_nodes(rod: Rod, folded_alpha_rad: FloatArray) -> FloatArray:
    """Use the folded m=0 chart and signed nonzero-rod polar-line chart."""

    alpha = np.asarray(folded_alpha_rad, dtype=np.float64)
    if alpha.ndim != 1 or np.any((alpha < 0.0) | (alpha > np.pi)):
        raise ValueError("folded_alpha_rad must be a vector in [0, pi]")
    if rod.family_m == 0:
        return alpha
    return np.concatenate((-alpha[alpha > 0.0][::-1], alpha))
...
def sample_scale_resolved_reciprocal_space(
    ...
    *,
    ...
    for rod in rods:
        rod_alpha_rad = _rod_display_angle_nodes(rod, alpha_nodes_rad)
        axial = structure_resolved_axial_nodes_Ainv(
            inputs,
            rod,
            background_count=u_background_count,
        )
        logical_candidate_count += rod_alpha_rad.size * beta.size * axial.size
        ...
        reserve_alpha = np.repeat(rod_alpha_rad[best_alpha], beta.size)
        ...
        if fill_evaluation_count:
            ...
            fill_evaluated = inputs.bragg_space.evaluate_latent(
                rod=rod,
                alpha_rad=rod_alpha_rad[fill_alpha_index],
...
def _sample_schematic_rays(
    ...
    *,
    ...
    for rod in nominal.rods:
        if rod.family_m == 0:
            continue
        signed_alpha = _rod_display_angle_nodes(rod, alpha_nodes_rad)
        alpha_grid, beta_grid = np.meshgrid(signed_alpha, beta, indexing="ij")
        for branch in (1, 2):
...
def _render_four_panel(data: PublicationData) -> object:
    ...
    caption_axis.text(
        ...
        0.12,
        (
            f"Panel a uses {data.mosaic_alpha_nodes_rad.size} folded magnitude nodes; nonzero rods "
            "use their signed rod-local lift while m=0 remains folded. Panel b "
...
def _publication_manifest_payload(
    ...
    *,
    ...
    return {
        "schema_version": "rasim-bi2se3-publication-mapping-v7",
        ...
        "display_policies": {
            "mosaic_alpha": "compiled_folded_m0_nodes_with_signed_nonzero_lift.v2",
...
def _ewald_only_manifest_payload(
    ...
    *,
    ...
    return {
        "schema_version": "rasim-bi2se3-detector-visible-ewald-v3",
        "config_path": provenance.config_identity,
        "config_sha256": provenance.config_sha256,
        "renderer_path": provenance.renderer_identity,
        "renderer_sha256": provenance.renderer_sha256,
...
def _schematic_only_manifest_payload(
    ...
    *,
    ...
    detector_norm = _detector_norm(data.ideal_detector, data.tilted_detector)
    return {
        "schema_version": "rasim-bi2se3-projection-schematic-v3",
        "config_path": provenance.config_identity,
        "config_sha256": provenance.config_sha256,
        "renderer_path": provenance.renderer_identity,
        "renderer_sha256": provenance.renderer_sha256,
        ...
        "display_policies": {
            ...
            "guide_rays": "signed_nonzero_polar_line_common_exit_correspondence.v2",
