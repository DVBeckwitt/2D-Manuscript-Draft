...
def project_bi2se3_ordered_artifact(
    ...
    *,
    ...
    response_revisions = document.get("response_revisions")
    if (
        ...
        or not isinstance(response_revisions, list)
        or len(response_revisions) != len(response_execution)
        or any(
            not isinstance(revision, str) or re.fullmatch(r"[0-9a-f]{64}", revision) is None
            for revision in response_revisions
        )
        or any(
