"""Normalized folded-m0/signed-nonzero mosaic space and fixed-u pushforward."""
...
class MosaicSpace:
    """Deterministic folded-m0 and signed nonzero-rod mosaic quadrature."""
    ...
    beta_rad: FloatArray
    m0_rotation_crystal: FloatArray
    probability_mass: FloatArray
    quadrature_rule_id: str = "midpoint_uniform_beta.v1"
    model_id: str = ROD_POLAR_LINE_MOSAIC_MODEL_ID
    coordinate_semantics_id: str = ROD_POLAR_LINE_COORDINATE_SEMANTICS_ID
    probability_measure_id: str = ROD_POLAR_LINE_PROBABILITY_MEASURE_ID
    ...
    def __post_init__(self) -> None:
        ...
        rotations = readonly_float_array(
            self.m0_rotation_crystal,
            (size, 3, 3),
            "m0_rotation_crystal",
        )
        ...
        object.__setattr__(self, "m0_rotation_crystal", rotations)
    ...
    def mosaic_slice(self, *, rod: Rod, u_Ainv: float) -> MosaicSlice:
        ...
        if rod.family_m == 0:
            ...
            q_crystal = u_value * mean_axis
            rotated_crystal = np.einsum(
                "nij,j->ni",
                self.m0_rotation_crystal,
                q_crystal,
                optimize=True,
            ...
            from painted_ewald.bragg import map_rod_polar_line_latent

            nonzero = self.alpha_rad > 0.0
            signed_tilt = np.concatenate(
                (
                    self.alpha_rad[~nonzero],
                    self.alpha_rad[nonzero],
                    -self.alpha_rad[nonzero],
                )
            )
            beta = np.concatenate(
                (
                    self.beta_rad[~nonzero],
                    self.beta_rad[nonzero],
                    self.beta_rad[nonzero],
                )
            )
            probability_mass = np.concatenate(
                (
                    self.probability_mass[~nonzero],
                    0.5 * self.probability_mass[nonzero],
                    0.5 * self.probability_mass[nonzero],
                )
            )
