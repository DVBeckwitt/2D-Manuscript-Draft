...
\end{equation}
Thus the strong $r=0$ oscillations are finite-thickness fringes. More generally, $h+2k=3m$, with integer $m$, gives $\omega_{hk}=1$ and $F_+=F_-$, so the same independence from the transition law holds for every such rod. This includes the $r=0$ and $r=3$ families. Their intensities are unchanged by the included registry and orientation changes at fixed stack length and trilayer structure. Figure~\ref{fig:si_pbi2_transition_validation} reserves the direct numerical checks of these limits. Figure~\ref{fig:si_pbi2_sensitivity} separates the effects of faults within a stack from changes in population weights.
