...
| `sections/refinement_workflow.tex` | IN PROGRESS | Moved to the end before the conclusion and expanded to explain staged instrumental, sample, mosaic, ordered, and PbI2 diffuse-scattering parameters. |
| `sections/results_ordered.tex` | IN PROGRESS | Now uses the moved mosaic/star discussion as motivation and focuses on the ordered Bi2Se3/Bi2Te3 validation payoff. |
| `sections/results_diffuse_pbi2.tex` | IN PROGRESS | Reintroduced as a draft extension with placeholders for detector image, stacking-fault model, reciprocal-space diffuse cylinders, and selected-trajectory comparison. |
...
| Add early two-component mosaicity evidence | IN PROGRESS | Text + figures | Raw detector motivation placeholder and moved low-L m=0 star-feature figure now appear at the start of the mosaicity section; full narrow-core versus Lorentzian-tail evidence can still be added if required. |
...
| Task | Status | Reason |
|---|---|---|
| PbI2 diffuse scattering / stacking-fault section | IN PROGRESS | Draft section reintroduced after ordered results and before workflow; keep subordinate to ordered-film validation. |
| PbI2 selected-rod validation figures | TODO | Placeholder added for measured/ordered/faulted/residual projection; generate real figure later. |
...
| Two-component mosaicity | Working | Early evidence drafted | Raw detector and low-L m=0 placeholders now motivate the Gaussian-core plus Lorentzian-tail text. |
| h-BN fitting method | Working or near working | Needs supplement writeup | Explain because it is part of the workflow. |
| Low-L m=0 star-feature ordering | Working | Ready for advisor review | Figure and discussion now sit in the mosaicity section before the correlated-effects tutorial and validation overlay. |
...
7. Generate real PbI2 detector/stacking/projection figures after the ordered-film story remains readable.
...
## 2026-05-12 advisor-feedback structural revision

- Moved the low-L $m=0$ star-feature figure from the ordered-results section into the mosaicity section.
- Added a raw detector-image placeholder immediately before the moved star-feature figure.
