...
centers.  Expanding $\langle|A_N|^2\rangle$, grouping the two orders of each
layer pair as complex conjugates, and dividing by $N$ gives the exact reduced
finite-stack expression
\begin{equation}
\begin{aligned}
  K_{m,\Delta}^{(j)}&=
  \boldsymbol{\pi}_{j,m}D_{F^*}M_j(\omega)^\Delta\mathbf{f},\\
  I_j(L)&=\frac{1}{N}\sum_{m=0}^{N-1}
  \boldsymbol{\pi}_{j,m}\mathbf{u}
  +\frac{2}{N}\operatorname{Re}\!\sum_{0\le m<n\le N-1}
  \xi(L)^{n-m}K_{m,n-m}^{(j)}.
\end{aligned}
\label{eq:pbi2_finite_intensity}
\end{equation}
Here $\mathbf{f}=(F_+,F_-)^{\mathsf T}$,
$\mathbf{u}=(|F_+|^2,|F_-|^2)^{\mathsf T}$,
$D_{F^*}=\operatorname{diag}(F_+^*,F_-^*)$, and
$\boldsymbol{\pi}_{j,m}=\boldsymbol{\pi}_{j,0}P_j^m$ is the orientation
population at trilayer $m$.  The first line defines the pair kernel:
$\boldsymbol{\pi}_{j,m}$ selects the earlier orientation, $D_{F^*}$ supplies
its conjugated form factor, $M_j^\Delta$ sums every allowed path and registry
phase across the intervening interfaces, and $\mathbf f$ supplies the later
form factor.  In the second line, the first term is self-scattering from the
$N$ trilayers and the second is the interference from every distinct pair.
The factor $\xi^{n-m}$ gives the vertical phase separation, while
$2\operatorname{Re}$ includes the reverse-ordered conjugate pair without
