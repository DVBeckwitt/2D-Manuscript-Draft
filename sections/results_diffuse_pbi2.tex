...
\end{equation}
For a fixed $(h,k)$ rod, a forward registry step
...
forward and backward registry shifts without an orientation change; and
...
projected I--Pb--I stacks in the same $rF_\sigma$ notation.  With $+c$ upward,
the sequences are read from bottom to top: $0F_+\rightarrow0F_+$ for 2H,
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\linewidth]{figures/results_pbi2/transition_matrix/pbi2_polytype_stacks.pdf}
  ...
  trilayer.  Nominal ionic signs are shown on the reference $F_+$ trilayer.}
...
$M(\omega)^\Delta$ gives the phase-weighted correlation between trilayers
separated by $\Delta$ interfaces.  The complete $6\times6$ construction,
exact reduction, and finite-stack self-plus-pair intensity are derived in
Supplementary Note S3.

For each polytype-rich population $j\in\{2H,4H,6H\}$, the five interface
probabilities are constrained by
\begin{equation}
  \boldsymbol{\theta}_j(\epsilon_j)
  =(1-\epsilon_j)\mathbf{e}_j+\epsilon_j\mathbf{q}_j,
  \label{eq:pbi2_fault_parameterization}
\end{equation}
where $\mathbf{e}_j$ is the deterministic parent event, $\epsilon_j$ is the
per-interface probability of departing from that event, and $\mathbf{q}_j$ is
a fixed normalized fault template.  In the present implementation,
$\mathbf{q}_j$ divides the fault probability equally among the four
non-parent events.  Separate 2H-, 4H-, and 6H-rich populations are added
incoherently,
\begin{equation}
  I_{\mathrm{calc}}(L)=
  S\left[\left(\sum_{j\in\{2H,4H,6H\}}w_j
  I_j(L;\epsilon_j,N)\right)\!\otimes R(L)\right]+B(L),
  \qquad \sum_j w_j=1,
  \label{eq:pbi2_population_sum}
\end{equation}
where $R(L)$ is the fixed instrumental/profile resolution and $B(L)$ is the
declared background model.  Thus $\epsilon_j$ is an effective disorder
probability conditional on the chosen fault template, and $w_j$ is an
effective population weight under the same structural, resolution, and scale
model.
...
\begin{table}[H]
  ...
  Eqs.~\eqref{eq:pbi2_fault_parameterization} and
  \eqref{eq:pbi2_population_sum}; values from the superseded scalar model are
  not reused.}
