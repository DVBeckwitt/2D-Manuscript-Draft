...
\end{equation}
Equations~\eqref{eq:bragg_matching} and~\eqref{eq:exit_wavevector} define the ideal integer Bragg-point limit. As developed later, replacing the integer order $\ell$ by a continuous coordinate $L$ generalizes the same construction to a reciprocal rod, with the Bragg orders lying at $L=\ell$.

Figure~\ref{fig:waxs_geometry_system} shows how an accepted exit direction reaches the area detector. The scattering angle $2\theta$ and outgoing-ray azimuth $\phi_f$ locate $\mathbf{k}_f$, while detector distance $D$, axes $x'$ and $z'$, normal $\hat{\mathbf n}_{\mathrm{det}}$, and tilts $\beta_D$ and $\gamma_D$ determine the detector location. The detailed ray--plane and pixel-coordinate equations are given in the Supporting Information.

\begin{figure}[H]
  \centering
  \resizebox{0.6\linewidth}{!}{\input{figures/geometry/system_geometry}}
  \caption{Laboratory and detector geometry. The incident wavevector $\mathbf{k}_i$ strikes the film, and a representative scattered wavevector $\mathbf{k}_f$ reaches the area detector at scattering angle $2\theta$ and outgoing-ray azimuth $\phi_f$. The detector axes $x'$ and $z'$, normal $\hat{\mathbf n}_{\mathrm{det}}$, distance $D$, and detector tilts define the ray-to-detector map.}
  \label{fig:waxs_geometry_system}
\end{figure}

Source position and propagation angles arising from divergence are sampled from the measured Gaussian beam distributions, while wavelength is sampled from the retained spectrum. The Supporting Information defines the weighted source-state construction. The external ray intersects the measured sample geometry, and refraction gives the internal vector $\mathbf{k}'_i$. Accepted exit rays must intersect the front face within the active detector bounds. Sample acceptance and detector acceptance are therefore consecutive geometric restrictions.
...
\end{equation}
Members with the same $r$ have the same in-plane radius. Their amplitudes are evaluated individually, and their intensity contributions are summed to represent scattering from the radial family. The family label is distinct from crystallographic multiplicity, and no second multiplicity factor is applied after this explicit sum.
...
The complex amplitude $F_{hk}$ is the structural input evaluated along the reference rod. The scalar object propagated through the remaining calculation is the continuous rod intensity $S_{hk}(L,\lambda_s)$. For an ordered finite film, $S_{hk}$ is constructed from $|F_{hk}|^2$. $S_{hk}(L,\lambda_s)\,dL$ is the integrated structural intensity carried by a small segment of the unrotated $(h,k)$ rod.
...
\end{equation}
Thus, every off-specular rod sweeps out a cylinder about the film-normal reciprocal-space axis, while the specular rod remains unchanged. Members with the same $r$ share the same geometric cylinder, although their amplitudes are evaluated separately. For off-specular cylinders with $Q_R>0$, the circumference factor $1/[2\pi Q_R(r)]$ preserves the integrated intensity when a rod is distributed around that cylinder. This construction produces the paired off-specular trajectories in Fig.~\ref{fig:intro_2x3}(h) while leaving the specular peaks behaving like single crystals.
