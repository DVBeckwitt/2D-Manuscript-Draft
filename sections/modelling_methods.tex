...
Figure~\ref{fig:waxs_geometry_system} shows how an accepted exit direction reaches the area detector. In the laboratory frame, $y$ follows the nominal incident beam, $z$ is vertical, and $x$ is transverse. For each ray, the scattering angle $2\theta$ is the angle between $\mathbf{k}_i$ and $\mathbf{k}_f$. The outgoing-ray azimuth $\phi_f$ is measured about the incident direction, with zero toward $+z$ in the reference geometry. Displayed detector angles use the nominal incident beam as their common reference.
...
\end{equation}
Members with the same $r$ have the same in-plane radius $Q_R(r)$ before mosaic tilt. Their amplitudes are evaluated individually, and their intensity contributions are summed to represent scattering from the radial family. The family label is distinct from crystallographic multiplicity, and the explicit sum accounts for multiplicity.
...
\end{equation}
Here $o_a$, $f_a$, $T_a$, and $\mathbf r_a$ are the occupancy, atomic scattering factor, Debye--Waller factor, and Cartesian site position. The factor $T_a$ accounts for atomic displacements and multiplies the scattering amplitude. Supporting Information Sec.~S2 defines the displacement parameters and their directional dependence.
