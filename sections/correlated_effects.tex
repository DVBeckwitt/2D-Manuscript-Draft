\subsection{Bandwidth-coupled refraction and absorption}
...
intersection alone. Each ray is transmitted into the film, refracted by the
material optical constants, attenuated during propagation, scattered from the
mosaic population, and refracted back into air before reaching the detector.
The forward model therefore treats bandwidth, refraction, transmission,
absorption, divergence, and mosaicity as coupled physical effects.

The coupling starts with the refractive index. For each sampled wavelength
...
that determine refraction and attenuation. The attenuation itself is not
applied through a separate scalar absorption coefficient; it enters through the
...
\end{equation}
Following the wave-propagation construction used in the dissertation, the
transmitted angle and internal wavevector scale are
\begin{align}
  \theta_t(\lambda_j)
  &=
  \cos^{-1}\!\left[
...
wavevectors set the damping per unit length. The exponent is dimensionless, so
\(L_{\mathrm{in}}\) and \(L_{\mathrm{out}}\) must use the same length units as
\(k_{t,z}^{-1}\). For a uniformly scattering film of thickness \(t\), the same
idea may be written as a depth average,
\begin{equation}
...
\end{equation}
This attenuation is multiplied by the entrance and exit transmission factors,
the structure-factor intensity, and the mosaic weight before the ray is
...
\(\tilde n(\lambda_j)\). The near-critical low-\(L\) feature is therefore a
coupled optical and geometric effect, not a post-processing correction.

The specular-family profiles in this near-critical region are therefore
calculated with an optical-to-structural handoff rather than with a purely
kinematic rod intensity. The low-\(Q_z\) branch is calculated with a Parratt
...
is matched to a normalized finite-stack structure term divided by \(Q_z^2\).
The two limits are joined with a smooth log-intensity blend over the region
where the optical and structural branches agree, as summarized in
Fig.~\ref{fig:parratt_handoff}. This construction prevents the near-critical
specular intensity from being treated as an ordinary high-\(Q_z\) structure
factor, while preserving the finite-stack modulation that controls the
higher-\(L\) rod profile. Details of the recursion, scale estimate, and blend
criterion are provided in the Supporting Information.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\linewidth]{figures/correlated_effects/fig_parratt_ht_handoff.png}
  \caption[Optical-to-structural handoff for near-critical specular intensity]{Optical-to-structural handoff used for the near-critical
  specular-family calculation. (a) Pure Parratt reflectivity gives the
  low-\(Q_z\) optical response, including refraction, evanescence, and multiple
  reflection at the film interfaces. The high-\(Q_z\) branch is the normalized
  finite-stack structure term divided by \(Q_z^2\) and scaled to the optical
  branch in an overlap region. The final curve is a smooth log-intensity blend
  between these limits, plotted versus \(Q_z/Q_c\) with the corresponding
  crystallographic \(L\) coordinate on the top axis.  
  \(Q_z\).}
...
\begin{figure}[H]
  ...
  \caption[Bandwidth/refraction mosaic-cap geometry for the low-L feature]{Bandwidth, refraction, and mosaic-cap geometry for the low-\(L\)
