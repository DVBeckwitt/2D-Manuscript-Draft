...
The remaining stages refine peak shape and intensity. Local detector profiles constrain the shared Gaussian-plus-Lorentzian mosaic distribution. Selected Bragg-region integrals constrain ordered structure-factor parameters, and the final validation projects measured and calculated images along the same fixed-$m$ trajectories for comparison versus $Q_z$. Implementation details including uncertainty due to $2\theta$--$\phi$ and $Q$ remapping, parameter correlations, lookup tables, and Monte Carlo sampling, are described in the supporting information. 
