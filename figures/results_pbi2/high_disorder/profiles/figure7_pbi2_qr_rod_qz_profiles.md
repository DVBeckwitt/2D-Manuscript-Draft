...
Source state: `C:\Users\Kenpo\.local\share\ra_sim\high_disorder.json`
...
Detector-rotation calibration is Qr-driven for centerline overlay alignment: success=`False`, non-specular fitted peaks=`0`, active peaks=`0`.
Detector distance for rod Q-space: `0.075` m -> `0.075` m.
Qr RMS: `nan` -> `nan` Å^-1; Qz diagnostic RMS: `nan` -> `nan` Å^-1.
Displayed/integrated detector support is limited to `2theta <= 70.3°` and `Qz > 0`.
Phi signs: `-` and `+`.
...
m = 0 ROI uses caked phi/2theta bounds `phi=[-10, 10]°`, `2theta=[5, 25]°`; its final L axis is autoscaled from the accepted ROI trace.
...
|---:|:---|:---:|---:|---:|---:|:---|---:|
| 1 | saved_q_group_rows | False | 1.5807 | 0 | 0 | saved_q_group_rows_no_fit_points | 0 |
| 3 | saved_q_group_rows | False | 2.7378 | 0 | 0 | saved_q_group_rows_no_fit_points | 0 |
| 4 | saved_q_group_rows | False | 3.1613 | 0 | 0 | saved_q_group_rows_no_fit_points | 0 |
| 7 | saved_q_group_rows | False | 4.182 | 0 | 0 | saved_q_group_rows_no_fit_points | 0 |
...
| gamma_deg | 4.69299 | 4.69299 |
| Gamma_deg | 0.00275231 | 0.00275231 |
