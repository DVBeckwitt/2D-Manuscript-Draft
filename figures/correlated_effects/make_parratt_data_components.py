...
gap = (L_data >= 0.94) & (L_data <= 1.08)
left = np.where(L_data < 0.94)[0][-1]
right = np.where(L_data > 1.08)[0][0]
I_data[gap] = 10 ** np.interp(
    L_data[gap], [L_data[left], L_data[right]],
    [np.log10(I_data[left]), np.log10(I_data[right])]
)
background = np.nanmin(I_data[(L_data >= 0.75) & (L_data <= 1.15)])
I_data = np.clip(I_data - background, 1e-8, None)
I_data /= np.nanmax(I_data[(L_data >= 0.19) & (L_data <= 1.05)])
...
I_parratt[(L_model >= 0.223) & (L_model <= 0.336)] = np.nan
finite = np.isfinite(I_parratt) & (I_parratt > 0)
I_parratt = 10 ** np.interp(
    L_model, L_model[finite], np.log10(I_parratt[finite])
)
...
plateau = np.nanmax(raw_kinematic[(L_model >= 0.53) & (L_model <= 0.55)])
I_kinematic[(L_model >= 0.40) & (L_model < 0.54)] = plateau

fig, ax = plt.subplots(figsize=(10.5, 5.9), constrained_layout=True)
data_range = (L_data >= 0.19) & (L_data <= 1.05)
model_range = (L_model >= 0.19) & (L_model <= 1.05)
