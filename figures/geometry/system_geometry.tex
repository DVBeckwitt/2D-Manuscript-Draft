...
\begin{tikzpicture}[tdplot_main_coords, scale=1.5]
    ...
    % Draw partial back face
    \draw[very thick]
      (detBBR) -- (detBTR) -- (detBTL);
    % Bottom back edge
    \draw[very thick] (detBBL) -- (detBBR);
    ...
    \draw[very thick] (detFBL) -- (detBBL);

    % Detector label
    \node at ($(detCenter)+(0,0,1.3)$) {\tiny Detector Screen};
    ...
    % Line related to phi, aligned with detector vertical
    \coordinate (phiLineEnd) at ($ (detCenter) +
        ( {0.6*\sina*\cosb}, {0.6*\sina*\sinb}, {0.6*\cosa}+0.2 ) $);
    \draw[cyan, dashed] (detCenter) -- (phiLineEnd);
    ...
    \node[scale=0.7, blue] at (0.0,0.12,-0.01) {\tiny $\vec{k}_i$};

    %=========================
    % Angle arcs for phi and theta
    %=========================
    \tdplotsetrotatedcoords{0}{90}{90}
    \tdplotdrawarc[dashed,tdplot_rotated_coords, cyan]
      {(0,0,-3)}{0.8}{20}{90}{anchor=north west}{\tiny $\phi$}

    \tdplotsetrotatedcoords{90}{-20.56}{0}
    \tdplotdrawarc[dashed, green,tdplot_rotated_coords, line width=0.1mm]
      {(0,1.4,0)}{0.5}{15}{90}
      {anchor=south east}{\scriptsize $\theta$}
