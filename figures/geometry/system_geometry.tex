...
\draw[detNormalArrow] (detNormalBase) -- (detNormalTip)
  ...
  {$\hat{\mathbf n}_{\mathrm{det}}$};
    ...
    \node[orange, scale=0.6, anchor=south east,
          xshift=6pt, yshift=-5pt] at (betaLabel) {$\beta_D$};
    ...
    \node[oiPurple, scale=0.6, anchor=south west,
          xshift=-18pt, yshift=-5pt] at (gammaLabel) {$\gamma_D$};

    %=========================
    % Sample and beams
    %=========================
    % Undeflected incident direction from sample to detector center
    \draw[black, dash dot] (-0.5,0,0) -- (detCenter);

    % Nominal distance in the untilted reference pose. Extension lines keep
    % the dimension separate from the incident and scattered ray paths.
    \coordinate (distanceSample) at (-0.5,-1.3,0.05);
    \coordinate (distanceDetector) at (-3,-1.3,0.05);
    \draw[gray,thin] (-0.5,0,0) -- (distanceSample);
    \draw[gray,thin] (detCenter) -- (distanceDetector);
    \draw[<->,black,thin] (distanceSample) -- (distanceDetector)
      node[midway,fill=white,inner sep=1pt,scale=0.65] {$D$};
    ...
    \node[scale=0.7, red, xshift=12pt, yshift=-2pt]
      at ($ (-0.5,0,0)!0.6!(scatP) $) {\tiny $\mathbf{k}_f$};

    % Incident vector k_i
    \draw[blue,->] (0.5,0,0) -- (-0.5,0,0);
    \node[scale=0.7, blue, yshift= -0.5mm] at (0.0,0.12,-0.01) {\tiny $\mathbf{k}_i$};

    %=========================
    % Angle arcs for phi and 2theta
    %=========================
    % Both arcs are derived from the drawn ray, not display-only angles.
    % Native TikZ coordinates (X,Y,Z) correspond to laboratory (-y,x,z).
    % The depicted hit has negative signed phi_f in the image convention.
    \pgfmathsetmacro{\hitRadius}{sqrt(\offY*\offY+\offZ*\offZ)}
    \pgfmathsetmacro{\phiMagnitude}{atan2(\offY,\offZ)}
    \pgfmathsetmacro{\twoTheta}{atan2(\hitRadius,2.5)}
    \draw[dashed,cyan!75!black,line width=0.15mm]
      plot[domain=0:\phiMagnitude,samples=40,variable=\t]
        (-3,{0.72*sin(\t)},{0.72*cos(\t)});
    \node[cyan!75!black,scale=0.65,fill=white,inner sep=1pt]
      at (-3,{0.63*sin(0.5*\phiMagnitude)},{0.63*cos(0.5*\phiMagnitude)}) {$\phi_f$};

    \draw[green!50!black,line width=0.15mm]
      plot[domain=0:\twoTheta,samples=30,variable=\t]
        ({-0.5-1.05*cos(\t)},
         {1.05*sin(\t)*\offY/\hitRadius},
         {1.05*sin(\t)*\offZ/\hitRadius});
    \node[green!50!black,scale=0.65,anchor=south east,inner sep=1pt]
      at ({-0.5-1.05*cos(0.5*\twoTheta)},
          {1.05*sin(0.5*\twoTheta)*\offY/\hitRadius},
          {1.05*sin(0.5*\twoTheta)*\offZ/\hitRadius}) {$2\theta$};
